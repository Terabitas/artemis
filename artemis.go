...
import (
	...
	"github.com/nildev/artemis/server"
	"github.com/nildev/artemis/version"
	"github.com/rakyll/globalconf"
)

const (
	DefaultConfigFile = "/etc/artemis/artemis.conf"
)
...
func main() {
	...
	userset := flag.NewFlagSet("artemisd", flag.ExitOnError)

	printVersion := userset.Bool("version", false, "Print the version and exit")
	cfgPath := userset.String("config", DefaultConfigFile, fmt.Sprintf("Path to config file. artemisd will look for a config at %s by default.", DefaultConfigFile))
	...
	cfgset := flag.NewFlagSet("artemisd", flag.ExitOnError)
	// Generic
...
func getConfig(flagset *flag.FlagSet, userCfgFile string) (*config.Config, error) {
	opts := globalconf.Options{EnvPrefix: "ARTEMISD_"}
